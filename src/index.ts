...
// 构造响应
function makeRes(body: any, status = 200, headers: { [key: string]: string } = {}): Response {
...
async function ADD(envadd: string): Promise<string[]> {
    ...
    const trimmed = addtext.charAt(0) === ',' ? addtext.slice(1) : addtext
    const final = trimmed.charAt(trimmed.length - 1) === ',' ? trimmed.slice(0, trimmed.length - 1) : trimmed
    return final.split(',')
}
...
// 首页路由
app.get('/', async (c) => {
    const userAgent = c.req.header('User-Agent')?.toLowerCase() || "null"
    const url = new URL(c.req.url)
    
    // 处理UA屏蔽
    if (c.env.UA) {
        屏蔽爬虫UA = 屏蔽爬虫UA.concat(await ADD(c.env.UA))
    }
    ...
    const hostname = url.searchParams.get('hubhost') || c.req.header('host') || ''
    ...
    // 处理重定向
    if (c.env.URL302) {
        return c.redirect(c.env.URL302, 302)
    } else if (c.env.URL) {
        if (c.env.URL.toLowerCase() === 'nginx') {
            return new Response(await nginx(), {
                headers: { 'Content-Type': 'text/html; charset=UTF-8' }
            })
        } else {
            return fetch(new Request(c.env.URL, c.req.raw))
        }
    } else {
        if (fakePage) {
...
app.all('*', async (c) => {
    ...
    if (url.pathname.includes('/token')) {
        const tokenUrl = AUTH_UPSTREAM + url.pathname + url.search
    ...
    if (url.pathname.startsWith('/v2/') && 
        ...
        if (repo) {
            const tokenUrl = `${AUTH_UPSTREAM}/token?service=registry.docker.io&scope=repository:${repo}:pull`
            const tokenResp = await fetch(tokenUrl, {
                headers: buildHeaders(c, 'auth.docker.io')
            })
            
            const tokenData = await tokenResp.json()
            const token = tokenData.token
            
            const headers = buildHeaders(c, hub_host)
...
async function cloneUpstream(resp: Response, c: any): Promise<Response> {
    ...
    if (h.has('location')) {
        ...
        if (loc.startsWith('https://registry-1.docker.io')) {
            h.set('location', loc.replace('https://registry-1.docker.io', selfHost(c)))
        }
    }
